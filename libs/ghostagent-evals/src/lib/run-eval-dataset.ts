...
export async function runEvalDataset({
  appendRequestBody,
  datasetPathOverride,
  passRateThresholdOverride,
  writeHistory
}: {
  appendRequestBody?: (testCase: EvalCase) => Record<string, unknown>;
  ...
  const passRateThreshold =
    passRateThresholdOverride ??
    Number(process.env.AGENTFORGE_EVAL_PASS_THRESHOLD ?? '0.8');
  ...
  const datasetPath = resolve(
    process.cwd(),
    datasetPathOverride ??
      process.env.AGENTFORGE_EVAL_DATASET_PATH ??
  ...
  const passRate = passed / Math.max(dataset.length, 1);
  ...
  const reportWithDelta = {
    ...report,
    regression: {
      passRateDelta: previousSnapshot
